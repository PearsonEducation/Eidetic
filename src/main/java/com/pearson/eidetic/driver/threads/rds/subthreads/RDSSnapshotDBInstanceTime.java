...
public class RDSSnapshotDBInstanceTime extends RDSSubThreadMethods implements Runnable, RDSSubThread {
    ...
    public RDSSnapshotDBInstanceTime(String awsAccessKeyId, String awsSecretKey, String awsAccountId, String uniqueAwsAccountIdentifier, Integer maxApiRequestsPerSecond,
            Integer numRetries, com.amazonaws.regions.Region region, ArrayList<DBInstance> DBInstanceTime) {
    ...
    public void run() {
        ...
        for (DBInstance dbInstance : DBInstanceTime_) {
            try {
                ...
                //Instance is not ready for snapshots
                if (!dbInstance.getDBInstanceStatus().equalsIgnoreCase("available")) {
                    logger.error("awsAccountNickname=\"" + uniqueAwsAccountIdentifier_ + "\",Event=\"Error\", Error=\"error in RDSSnapshotDBInstanceTime workflow "
                            + dbInstance.getDBInstanceIdentifier() + " instance is not in an available state.\"");
                    continue;
                }
    ...
    public boolean snapshotCreation(Region region, AmazonRDSClient rdsClient, DBInstance dbInstance, String period, Date date) {
        ...
        try {
            ...
            String description = period + "_snapshot " + dbInstance.getDBInstanceIdentifier() + " by Eidetic Time at " + date.toString();
    ...
    public ArrayList<DBInstance> printDBInstances() {
        return DBInstanceTime_;
    }
}
